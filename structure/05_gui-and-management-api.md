...
| Models | Fetch routed model lists, disabled model visibility, catalog-facing ids, and the global one-hop model alias map. |
...
| Models and visibility | `src/server/management/model-routes.ts` — `GET /api/models`, `GET/PUT /api/model-aliases`, `PUT /api/disabled-models`, `PUT /api/model-visibility`, `PUT /api/selected-models`, `GET/POST /api/custom-models`. Alias PUT replaces the top-level map on the shared live config and persists it without catalog convergence, so the next request sees the new one-hop route. Visibility writes trigger catalog sync through the owning server path. |
